...
parser = argparse.ArgumentParser(description='oms stable diffusion')
...
if args.faceid_version == "FaceID":
    ...
    pipe.fuse_lora()
    from garment_adapter.garment_ipadapter_faceid_stable import StableIPAdapterFaceID

    ip_model = StableIPAdapterFaceID(pipe, args.model_path, ip_ckpt, device, args.enable_cloth_guidance)
else:
    ...
    from garment_adapter.garment_ipadapter_faceid_stable import StableIPAdapterFaceIDPlus as StableIPAdapterFaceID

    garment_extractor_path = os.path.join(args.model_dir, "garment_extractor.safetensors")
    garment_ip_layer_path = os.path.join(args.model_dir, "ip_layer.pth")
