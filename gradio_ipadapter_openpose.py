...
parser.add_argument(
    ...
    type=str,
    default="FaceIDPlus",
    choices=["FaceID", "FaceIDPlus", "FaceIDPlusV2"],
)
...
if args.enable_cloth_guidance:
    pipe = OmsDiffusionControlNetPipeline.from_pretrained(
        args.pipe_path,
        vae=vae,
...
if args.faceid_version == "FaceID":
    ...
    ip_model = IPAdapterFaceID(
        pipe, args.model_path, ip_ckpt, device, args.enable_cloth_guidance
    )
else:
    ...
    from garment_adapter.garment_ipadapter_faceid import (
        IPAdapterFaceIDPlus as IPAdapterFaceID,
    )
    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Accordion("Advanced options", open=False):
                ...
                height = gr.Slider(
                    label="Height", minimum=256, maximum=1024, value=768, step=64
                )
                width = gr.Slider(
                    label="Width", minimum=192, maximum=768, value=576, step=64
                )
                ...
                guidance_scale = gr.Slider(
                    ...
                    minimum=1,
                    maximum=10.0,
                    value=3.0 if args.enable_cloth_guidance else 2.5,
                    step=0.1,
                )
                cloth_guidance_scale = gr.Slider(
                    label="Cloth guidance Scale",
                    minimum=1,
                    maximum=10.0,
                    value=3.0,
                    step=0.1,
                    visible=args.enable_cloth_guidance,
                )
                seed = gr.Slider(
