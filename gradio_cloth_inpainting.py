...
import torch
from diffusers import UniPCMultistepScheduler, AutoencoderKL, ControlNetModel, StableDiffusionControlNetInpaintPipeline
import gradio as gr
import argparse
...
args.enable_cloth_guidance = True
device = "cuda"
...
pipe = VirtualTryOnPipeline.from_pretrained(args.pipe_path, vae=vae, controlnet=control_net_inpaint, torch_dtype=torch.float16)
...
def process(cloth_image, cloth_mask_image, prompt, a_prompt, n_prompt, num_samples, width, height, sample_steps, scale, cloth_guidance_scale, seed, person_image_mask):
    ...
    images, cloth_mask_image = full_net.generate(cloth_image, cloth_mask_image, prompt, a_prompt, num_samples, n_prompt, seed, scale, cloth_guidance_scale, sample_steps, height, width, image=person_image,mask_image=person_mask,control_image=control_img)
...
with block:
    ...
    ips = [cloth_image, cloth_mask_image, prompt, a_prompt, n_prompt, num_samples, width, height, sample_steps, guidance_scale, cloth_guidance_scale, seed, person_image_mask]
