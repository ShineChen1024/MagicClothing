...
parser.add_argument(
    "--pipe_path", type=str, default="runwayml/stable-diffusion-inpainting"
)
...
pipe = OmsDiffusionInpaintPipeline.from_pretrained(
...
def process(
    ...
    images, cloth_mask_image = full_net.generate_inpainting(
        cloth_image,
        cloth_mask_image,
        num_samples,
        seed,
    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Accordion("Advanced options", open=False):
                ...
                height = gr.Slider(
                    label="Height", minimum=256, maximum=1024, value=1024, step=64
                )
                width = gr.Slider(
                    label="Width", minimum=192, maximum=768, value=768, step=64
                )
