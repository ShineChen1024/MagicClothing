...
def onnx_inference(lip_session, input_dir):
    ...
    with torch.no_grad():
        for _, batch in enumerate(tqdm(dataloader_lip)):
            ...
            parsing_result_lip = np.pad(parsing_result_lip, pad_width=1, mode='constant', constant_values=0)
            arm_mask = (parsing_result_lip == 14).astype(np.float32) \
                       + (parsing_result_lip == 15).astype(np.float32)
            upper_cloth_mask = (parsing_result_lip == 5).astype(np.float32) + arm_mask
            img = np.where(upper_cloth_mask, 255, 0)
            dst = hole_fill(img.astype(np.uint8))
            parsing_result_filled = dst / 255 * 5
            parsing_result_woarm = np.where(parsing_result_filled == 5, parsing_result_filled, parsing_result_lip)
            refine_hole_mask = refine_hole(parsing_result_filled.astype(np.uint8), parsing_result_lip.astype(np.uint8),
                                           arm_mask.astype(np.uint8))
            parsing_result_lip = np.where(refine_hole_mask, parsing_result_lip, parsing_result_woarm)
            parsing_result_lip = parsing_result_lip[1:-1, 1:-1]
            cloth_mask = refine_mask((parsing_result_lip == 5))
